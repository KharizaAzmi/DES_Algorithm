...
#menghitung xor dari dua string bilangan biner a dan
def xor(a, b):
    ans = ""
...
# Tabel Posisi 64 bit pada level awal: Tabel Initial Permutation
...
def decrypt(ct, rkb, rk):
    ...
    left = ct[0:32]
    right = ct[32:64]
    for i in range(15, -1, -1):  # Perulangan dibalik
        # Expansion D-box: Expand 32 bits data into 48 bits
        right_expanded = permute(right, exp_d, 48)

        # XOR RoundKey[i] dan right_expanded
        xor_x = xor(right_expanded, rkb[i])

        # S-boxes : Mengganti nilai dari tabel s-box dengan menghitung baris dan kolom
        sbox_str = ""
        for j in range(0, 8):
            row = bin2dec(int(xor_x[j * 6] + xor_x[j * 6 + 5]))
            col = bin2dec(int(xor_x[j * 6 + 1] + xor_x[j * 6 + 2] + xor_x[j * 6 + 3] + xor_x[j * 6 + 4]))
        ...
        # Swapper
        if i != 0:  # Untuk perulangan terakhir, tidak perlu ditukar
            left, right = right, left

        print("Round", 16 - i, bin2hex(left), bin2hex(right), rk[i])
...
#Tabel Kompresi Kunci : Kompresi kunci dari 56 bit hingga 48 bit
...
# cetak Plaintext dan Chipertext
print("Encryption")
chiper_text = bin2hex(encrypt(pt, rkb, rk))
print("Chiper Text : ", chiper_text)

print("Decryption")
text = bin2text(decrypt(chiper_text, rkb, rk))
